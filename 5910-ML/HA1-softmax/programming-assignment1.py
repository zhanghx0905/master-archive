...
def compute_softmax_loss(W, X, y, reg):
    ...
    def softmax(x):
        exp = np.exp(x - np.max(x))    # for numeric stability
        output = exp / (np.sum(exp, axis=1, keepdims=True) + EPSILON)
        return output
    ...
    y_onehot = onehot_encoding(y, K)   # N * K
    
    loss = np.mean(np.sum(- y_onehot * np.log(prob + EPSILON), axis=1))
    loss += 0.5 * reg * np.sum(W**2)

    dW = X.T.dot(prob - y_onehot) / N + reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
