...
public class BigramFrequencyStripes extends Configured implements Tool {
	...
	private static final String TOT_MARK = "";
	...
	private static class MyMapper extends
		...
		public void map(LongWritable key, Text value, Context context)
			...
			for (int i = 0; i < words.length - 1; ++i) {
				if (words[i].length() == 0 || set.contains(words[i])) {
					continue;
				}
				set.add(words[i]);
				KEY.set(words[i]);
	...
	private static class MyReducer extends
		...
		public void reduce(Text key,
			...
			float tot = SUM_STRIPES.get(TOT_MARK);
			BIGRAM.set(key.toString(), TOT_MARK);
			FREQ.set(tot);
			context.write(BIGRAM, FREQ);

			Set<String> secondkeys = SUM_STRIPES.keySet();
			secondkeys.remove(TOT_MARK);
			for (String v : secondkeys) {
				BIGRAM.set(key.toString(), v);
				FREQ.set(SUM_STRIPES.get(v) / tot);
				context.write(BIGRAM, FREQ);
			}
		}
	}
