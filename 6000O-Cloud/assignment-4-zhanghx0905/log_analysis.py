...
hourHostCount = hourGroupedHosts.map(
  lambda item: (item[0], len(set(item[1])))
)
...
avgHourlyReqPerHost = (sortedByHour.map(
  lambda item: (item[0], len(item[1]) // len(set(item[1])))
)).cache()
...
hoursWithAvg = [i[0] for i in avgHourlyReqPerHostList]
avgs = [i[1] for i in avgHourlyReqPerHostList]
...
badEndpointsTop10 = badEndpointsSum.takeOrdered(10, lambda x: -1 * x[1])
...
hoursWithErrors404 = [s[0] for s in errHourList]
errors404ByHours =  [s[1] for s in errHourList]
